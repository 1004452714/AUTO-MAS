...
class ProgressSnapshot:
    ...
    def file_percentage(self) -> float:
        """当前文件进度百分比（0–100）；``file_total_size <= 0`` 时返回 ``0.0``。"""
        if self.file_total_size <= 0:
            return 0.0
        return min(100.0, self.file_current_size / self.file_total_size * 100.0)
...
class ProgressBase:
    ...
    def set_counts(self, current_count: int) -> None:
        """更新已完成文件计数（不触发立即上报，随下次 ``emit`` 刷新）。

        Args:
            current_count: 已完成的文件数。
        """
        self._update(current_count=current_count)
    ...
    def finish(self) -> None:
        """收尾：速度/剩余时间归零，推送最终快照并通知 ``on_finished``。"""
        self._update(speed=0.0, time_left=0.0)
        self.listener.on_progress(self.snapshot())
        self.listener.on_finished(self.snapshot())
...
class ProgressListener:
    ...
    def on_finished(self, snapshot: ProgressSnapshot) -> None:  # pragma: no cover
        """完成回调，子类按需覆写。
    ...
    def on_message(self, message: str, level: str = "info") -> None:  # pragma: no cover
        """消息回调（如日志/提示），子类按需覆写。
...
class ConsoleProgressListener(ProgressListener):
    """把进度渲染到 stderr 单行（带回车刷新），适合 CLI。"""

    def __init__(self, stream=None, enabled: bool = True) -> None:
        """初始化控制台进度监听器（默认输出到 stderr，可由 ``enabled`` 关闭渲染）。"""
        self.stream = stream or sys.stderr
        self.enabled = enabled
        self._last_len = 0

    def on_progress(self, snapshot: ProgressSnapshot) -> None:
        """把进度渲染到 stderr 单行（回车刷新）。

        一行内依次是活动文案、总进度百分比、已下/总量、速率，剩余时间已知时
        追加 ``ETA``。
        ...
        if not self.enabled:
            return
        line = (
            f"\r{snapshot.activity} "
            f"{snapshot.percentage:6.2f}%  "
            f"{summarize_size(snapshot.current_size)}/{summarize_size(snapshot.total_size)}  "
            f"{summarize_size(snapshot.speed)}/s"
        )
        if snapshot.time_left > 0:
            line += f"  ETA {int(snapshot.time_left)}s"
        padding = max(0, self._last_len - len(line))
        self._last_len = len(line)
        self.stream.write(line + " " * padding)
        self.stream.flush()

    def on_finished(self, snapshot: ProgressSnapshot) -> None:
        """输出换行结束单行进度显示。

        Args:
            snapshot: 当前进度快照（此处未使用，保留接口一致性）。
        """
        if not self.enabled:
            return
        self.stream.write("\n")
    ...
    def on_message(self, message: str, level: str = "info") -> None:
        """输出一行消息，按级别加 ``[WARN]`` / ``[ERROR]`` 前缀。

        Args:
            message: 消息正文。
            level: 级别名（``info``/``warning``/``error``），默认 ``"info"``。
        """
        if not self.enabled:
            return
        if self._last_len:
...
@dataclass
class SpeedLimiter:
    """简单令牌桶限速器， / ``DownloadClient`` 限速。

    ``limit_bytes_per_sec <= 0`` 表示不限速。
    """

    limit_bytes_per_sec: float = 0.0
    _tokens: float = 0.0
    _updated: float = field(default_factory=time.monotonic, repr=False)

    @property
    def enabled(self) -> bool:
        """限速是否开启（``limit_bytes_per_sec > 0`` 时为真）。"""
        return self.limit_bytes_per_sec > 0

    def consume(self, size: int) -> None:
        """按令牌桶扣减令牌；限速开启时会**阻塞**（``time.sleep``）直到允许发送 ``size`` 字节。

        未开启限速（``limit_bytes_per_sec <= 0``）时立即返回，不做任何等待。
        ...
        self._tokens = min(
            ...
            self._tokens + (now - self._updated) * self.limit_bytes_per_sec,
        )
        self._updated = now
        self._tokens -= size
        if self._tokens < 0:
            time.sleep(-self._tokens / self.limit_bytes_per_sec)
