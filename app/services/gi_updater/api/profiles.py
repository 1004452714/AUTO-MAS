...
"""原神各区服的预设（Profile）：端点、launcher 三元组、渠道与语音 ID 表。
...
class PresetConfig:
    ...
    #: 语音语言 ID <-> locale code 映射（三游戏各不相同，见 docs/DESIGN.md §4）
    voice_language_id_map: Dict[int, str] = field(default_factory=dict)
    voice_locale_id_map: Dict[str, int] = field(default_factory=dict)
    ...
    launcher_plugin_url: str = ""
    launcher_game_channel_sdk_url: str = ""

    # 开关
    is_plugin_update_enabled: bool = False
    ...
    def locale_code_by_voice_id(self, voice_id: int) -> str:
        """把语音 ID 映射成 locale 码（如 ``0 -> "zh-cn"``）。

        Args:
            voice_id: 服务端/配置里的语音整数 ID。

        Returns:
            对应 locale 码；映射缺失时回退到 ``default_voice_language``。
        """
        return self.voice_language_id_map.get(voice_id, self.default_voice_language)

    def voice_id_by_locale_code(self, locale_code: str) -> int:
        """把 locale 码反查成语音 ID。

        Args:
            locale_code: 语音 locale 码（大小写不敏感）。

        Returns:
            对应整数 ID；未命中时回退到默认语音的 ID，仍无则 ``0``。
        """
        return self.voice_locale_id_map.get(
            str(locale_code).lower(),
            self.voice_locale_id_map.get(self.default_voice_language, 0),
        )
...
def _build_profiles() -> Dict[Tuple[str, str], PresetConfig]:
    ...
    # ---------------------------------------------------------------- 原神
    # 语音 ID：按官方接口的 zh-cn/en-us/ja-jp/ko-kr 顺序编号
    gi_voice_id = {0: "zh-cn", 1: "en-us", 2: "ja-jp", 3: "ko-kr"}
    gi_voice_locale = {v: k for k, v in gi_voice_id.items()}
    ...
    profiles[("gi", Region.CN)] = PresetConfig(
        ...
        voice_language_id_map=gi_voice_id,
        voice_locale_id_map=gi_voice_locale,
        api_base=_CN_API,
        downloader_base=_CN_DL,
        launcher_plugin_url=f"{_CN_API}/hyp/hyp-connect/api/getGamePlugins",
        is_plugin_update_enabled=True,
    ...
    profiles[("gi", Region.GLOBAL)] = PresetConfig(
        ...
        launcher_plugin_url=f"{_GLB_API}/hyp/hyp-connect/api/getGamePlugins",
        is_plugin_update_enabled=True,
        is_force_redirect_to_sophon=True,
    )
