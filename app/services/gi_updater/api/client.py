...
class HttpClient:
    ...
    def post_json(
        ...
        """POST 一份 JSON 并解析 JSON 响应（读取后自动关闭响应）。

        Args:
            url: 完整请求地址。
            payload: 可被 ``json.dumps`` 序列化的对象，作为请求体。
            headers: 附加请求头；默认会补上 ``Content-Type: application/json``，
                值为 ``None`` 时可用来**去掉**米哈游专用的默认头。
            timeout: 覆盖客户端默认超时的秒数；``None`` 时用默认值。
        ...
        Note:
            鹰角（Hypergryph）的 ``batch_proxy`` 就是这一型接口：单个 POST 拿到
            最新版本、整包与增量清单，无需签名或鉴权头。
        """
        merged: Dict[str, Optional[str]] = {"Content-Type": "application/json"}
        merged.update(headers or {})
        body = json.dumps(payload, ensure_ascii=False).encode("utf-8")
        response = self.request("POST", url, headers=merged, timeout=timeout, body=body)
        try:
            return response.json()
        finally:
            response.close()
    ...
    def head(self, url: str) -> HttpResponse:
        """发起 HEAD 请求（便捷封装，等价于 ``request("HEAD", ...)``）。

        Args:
            url: 请求地址。

        Returns:
            统一封装的 ``HttpResponse``（通常只用 ``status``/``headers``）。
        """
