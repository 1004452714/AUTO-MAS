...
"""宿主侧的更新编排：查版本、下载、校验、落盘、写回版本号——与具体游戏无关。

引擎（:mod:`app.services.gi_updater`）全同步（urllib + 线程 + 阻塞文件 IO）；本模块是
它接到 MAS 上的那只手：把整轮流程挪出事件循环、把下载线程里的进度快照翻译成一行行
调度台日志、并在联网算出计划之后补三道只有宿主才该管的门。一款游戏的门面只需要把
自己的短名与区服标签传进来。
...
_fresh_cache: dict[str, UpdateResult] = {}
...
def _cached_result(key: str) -> UpdateResult | None:
    """取该目录在短时窗口内的成功结论。
...
def _remember(key: str, result: UpdateResult) -> None:
    """记住这次成功结论，供同一目录的短时复用。
...
def _resolve_region(game: str, game_dir: Path, resource: str) -> str | None:
    ...
    Args:
        game: 游戏短名。
        game_dir: 游戏安装目录。
        resource: 配置里的区服（``自动`` 或该游戏的区服标签，如 ``官服``）。
...
async def update_client(
    ...
    *,
    ...
    Args:
        game: 游戏短名，如 ``gi``。
        game_path: 游戏安装目录（版本配置与可执行文件所在的那一级）。
    ...
    abort_event = threading.Event()
    key = f"{spec.key}:{Path(game_path)}".casefold()
    cached = _cached_result(key)
    if cached is not None:
        # 常态复用：只留 app.log 一行，不刷任务日志
        _note(
            f"{int(_MEMO_TTL_SEC // 60)} 分钟内已确认过{spec.display_name}"
            "客户端是最新的，本轮跳过检查"
        )
...
async def _run_update(
    ...
    *,
    ...
    if resolved is None:
        ...
        message = f"该目录里同时存在{labels}的可执行文件，请先确认游戏目录是否填错"
        _note(message)
        return UpdateResult(success=False, noop=True, message=message)
    ...
    try:
        ...
        blocked = await _run_gates(spec, plan, game_dir, updater, on_progress)
        ...
        logger.info("{}更新已中止，已下载的差分缓存会留给下次", display)
        return UpdateResult(
            success=False,
            aborted=True,
        ...
        logger.opt(exception=True).warning(
            "{}更新失败: {}: {}", display, type(error).__name__, error
        )
...
async def _run_gates(
    ...
    Args:
        spec: 本次更新所属游戏的装配信息，提供面向用户的名字。
        plan: 已算出的计划。
        game_dir: 游戏安装目录。
        updater: 引擎门面对象，提供该游戏的混装判定。
    ...
    if not updater.installer.validate_exec_data_dir():
        return await _block(
            plan,
            f"该目录疑似混装了不同区服的{spec.display_name}客户端，已停止更新",
        )
...
async def _block(plan: Any, message: str) -> UpdateResult:
