...
- **手动入口**（``manual=True``）：用户页「检查更新」由用户主动触发，跳过开关，
...
- **判不出来就放行**：路径解析不出、渠道认不出属于「无法判定」，旧客户端通常仍能
  登录，不该因为查不到版本就拦住用户；B 服与游戏正在运行同理，跳过而不是判失败；
...
#: 区服短名 -> 编排层接受的区服写法
_REGION_TO_LABEL: dict[str, str] = {"cn": "官服", "global": "国际服"}

#: 本地客户端进程名（与 BGI 侧口径一致），用来判「游戏正在运行」
_LOCAL_PROCESS_NAMES: tuple[str, ...] = ("YuanShen.exe", "GenshinImpact.exe")

#: 自动流程的单轮时限（分钟）：调度任务不能因为一次更新无限挂着。已完成的部分与
#: 下载缓存都会保留，下次从断点继续。手动入口不设时限——用户在场，要停可以自己按。
    ...
    *,
    ...
    Args:
        ...
        manual: 是否为用户页「检查更新」手动触发。``True`` 时忽略
            ...
            ``RuntimeError``——用户主动发起就该得到明确的失败原因，而不是像
            自动流程那样静默放行。
        should_abort: 中止判定，传给编排层在文件与数据块边界收工。

    Returns:
        是否可以继续本次任务。``True`` 含更新成功、无需更新、以及「判不出来所以
        不打扰」；``False`` 表示确知需要更新却没做成，要用户先处理。
        ``manual=True`` 时不会返回 ``False``，这类情况一律抛错。
    ...
    resource = _REGION_TO_LABEL.get(_CHANNEL_TO_REGION.get(channel or "", ""))
    if resource is None:
        if manual:
            raise RuntimeError(
    ...
    result = await update_genshin_client(
        str(game_exe.parent),
        resource=resource,
        time_limit_min=0 if manual else _AUTO_TIME_LIMIT_MIN,
        on_progress=report,
    ...
    if result.success:
        if result.noop:
            # 手动入口是用户主动来问的，「已是最新」必须当面回答；
            # 自动流程只留 app.log 一行，不占任务日志
            if manual:
                await _report(
